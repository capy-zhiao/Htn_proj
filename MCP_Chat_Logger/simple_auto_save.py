...
@mcp.tool()
async def save_current_chat() -> str:
    """
    Save current chat history automatically
    This tool doesn't require parameters and will save the current conversation
    """
    ensure_logs_directory()
    
    # Generate filename
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"chat_logs/current_chat_{timestamp}.md"
    
    # Create a simple chat history
    formatted_content = "# Current Chat History\n\n"
    formatted_content += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    formatted_content += "This is an automatically saved chat session.\n\n"
    formatted_content += "## Chat Summary\n\n"
    formatted_content += "The conversation was about MCP tools and chat logging functionality.\n\n"
    
    # Save file
    with open(filename, "w", encoding="utf-8") as f:
        f.write(formatted_content)
    
    return f"✅ Current chat saved to: {filename}"
